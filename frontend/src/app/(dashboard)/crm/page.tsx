...
export default function CRMContactsPage() {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      ...
      try {
        let url = `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/crm/customers`;
        if (filter !== "all") {
  ...
  const handleSaveCustomer = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingCustomer 
        ? `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/crm/customers/${editingCustomer.id}`
        : `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/crm/customers`;
  ...
  const handleDeleteCustomer = async (customerId: string) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const resp = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/crm/customers/${customerId}`, {
        method: "DELETE",
